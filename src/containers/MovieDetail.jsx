...
import { CastList, TrailerList, RelatedList} from '../components';
import { CAST_MAX_NUM, TRAILER_MAX_NUM, RELATED_MAX_NUM } from '../const';
import { Grid, Row, Col} from 'react-bootstrap/lib';
import { MovieInfo, Poster } from '../components';
import { connect } from 'react-redux';
import { fetchMovieDetail, fetchCastList, fetchTrailerList, fetchRelatedMovies} from '../actions';
...
class MovieDetail extends Component {
  ...
  render() {
    const {movie, casts, trailers, relateds, isFetcing_movie, isFetcing_casts, isFetcing_trailers, isFetcing_relateds} = this.props;
    if(isFetcing_movie || isFetcing_casts || isFetcing_trailers || isFetcing_relateds) {
      return <p>loading...</p>
    }
    if(movie.hasOwnProperty('id')) {
      return(
        <Grid fluid={false}>
          ...
          <Row >
            <RelatedList data={relateds.results.slice(0,RELATED_MAX_NUM)} />
          </Row>
        </Grid>
      );
    } else
...
function mapStateToProps(state){
  ...
  // My solution
  const {isFetcing_relateds, items: relateds, error_relateds} = relatedList;


  return {isFetcing_movie, movie, error_movie, isFetcing_casts, casts, error_casts, isFetcing_trailers, trailers, error_trailers, isFetcing_relateds, relateds, error_relateds}
}
